...
ggplot(mpd_grid) +
  ...
  theme(
    ...
    #line = element_blank(),
    #rect = element_blank()
    ) + labs(fill = "SES (more or less phylogenetically clustered than expected")
...
```{r AET_data_from_MODIS, echo =TRUE}
##### MODIS data for evapotranspiration  ####
modis1<-raster("data/NPP_AET_data/MODIS/MOD16A2.006_ET_500m_doy2013001_aid0001.tif") 
plot(modis1)
#kg/m2/8days 

m1<-extract(modis1, SA_grids) #extract data from raster
str(m1) #check structure, 1207 grids with 14400 data points each... 

#saveRDS(m1, "AET_data.rds")
##read in AET data
AET<-readRDS("AET_data.rds") 

mysummary <- data.frame(matrix(ncol=4,nrow=length(AET)))
colnames(mysummary) <- c("mean","median","min","max")
for(i in seq(length(AET))){
  AET[[i]][AET[[i]] > 32700] <- NA
  mysummary$mean[i] <- mean(AET[[i]], na.rm=T)
  mysummary$median[i] <- median(AET[[i]], na.rm = T)
  mysummary$max[i] <- max(AET[[i]], na.rm = T)
  mysummary$min[i] <- min(AET[[i]], na.rm = T)
}
...
References:
1. Zhang, K., J.S. Kimball, R.R. Nemani, S.W. Running, Y. Hong, J.J. Gourley, and Z. Yu, 2015. Vegetation Greening and Climate Change Promote Multidecadal Rises of Global Land Evapotranspiration, Scientific Reports, 5, 15956; doi: 10.1038/srep15956.
2. Zhang, K., J.S. Kimball, R.R. Nemani, and S.W. Running, 2010. A continuous satellite-derived global record of land surface evapotranspiration from 1983-2006. Water Resources Research doi:10.1029/2009WR008800.
3. Zhang, K., J.S. Kimball, Q. Mu, L.A. Jones, S.J. Goetz and S.W. Running, 2009. Satellite based analysis of northern ET trends and associated changes in the regional water balance from 1983 to 2005. Journal of Hydrology 379, 92-110.
```{r}
#http://files.ntsg.umt.edu/data/ET_global_monthly/Global_8kmResolution/
library(ncdf4)
nc<-"~/Documents/UBC/SA_trees/NPP_AET_data/Global_Monthly_ET_2013.nc"
et_raster<-brick(nc, varname = "monthly_ET")


```


#Surface leaf area data 
```{r source_for_SLA_data, include = FALSE}
#Botanical Information and Ecology Network
library(BIEN)
#make tree data names match BIEN names
species_names<-(gsub( "_", " ", tree_data$Scientific_name))
#remove sub species identities
sub("^(\\S*\\s+\\S+).*", "\\1", species_names)->species_names_clean
#search BIEN for the species names
BIEN_trait_species(species_names_clean) -> BIEN_data
#pull out surface leaf area measurements
subset(BIEN_data, trait_name == "leaf area per leaf dry mass")->SLA_data
unique(SLA_data$scrubbed_species_binomial) #only 79 of the 1085 species 
