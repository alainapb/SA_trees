...
contMap(corr_tree, shape_values, fsize = 0.2, lwd = 0.4, type= "fan") #odd, there are negative values... 
...
summary(lm(log(treeheight) ~ ntaxa, data = trait_npp_data)) 
cor.test(log(trait_npp_data$treeheight), trait_npp_data$ntaxa) #r = 0.709
plot(log(treeheight) ~ ntaxa, data = trait_npp_data) #taller trees in areas with more species 
```
...
full_model_obs<-lm(mpd.obs ~ log(treeheight) + log(leafshape) + ntaxa + NPP, data = trait_npp_data)
...
full_model_ses<-lm(mpd.obs.z ~ log(treeheight) + log(leafshape) + ntaxa + NPP, data = trait_npp_data)
...
globalMoran_obs #0.78  the residuals of the model are positively autocorrelated
#SES mpd
globalMoran_ses <- moran.test(full_model_ses$residuals, listw)
globalMoran_ses #0.82  the residuals of the model are positively autocorrelated
...
###
model_lat<-lm(mpd.obs.z ~ log(treeheight) + log(leafshape) + ntaxa + NPP + lat + long, data = trait_npp_data)
summary(model_lat)
plot(model_lat)
...
#bgr_data2<-bgr_data[,c(1,2,3,4,7,8,9)] # only include the traits for the model
# for reproducibility
set.seed(456)
...
## [1] 0.4720749 the model is off by ~.5 units of mpd.z
...
pred <- predict(gbm.fit2, n.trees = gbm.fit2$n.trees, bgr_data2)
# results
caret::RMSE(pred, bgr_data2$mpd.obs.z) #0.035  good model
```

#How well do the boosted regession model predictions match the data?
```{r plot_bgr_predictions, echo = TRUE}
#add predictions to data frame
bgr_data2$prediction<-pred

ggplot(bgr_data2)+
  geom_line(aes(y = mpd.obs.z, x = log_leafshape))+
  geom_line(aes(y =prediction, x = log_leafshape), color = "red")


plot(prediction ~ log_leafshape, bgr_data2, pch = 19, col = "red")
abline(h=0)

plot(prediction ~ log_treeheight, bgr_data2, pch = 19, col = "red")
abline(h=0)
