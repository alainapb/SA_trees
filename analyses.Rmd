...
fig2a<-ggplot(corr_data, mapping=aes(x= log(ratio), y = log(height)))+
  ...
  geom_smooth(method = "lm", color = "black", fill = "#ffcc33", alpha = 0.5)+
...
fig2b<-corr_data %>%
  ggplot()+
  geom_point(mapping=aes(x=log(ratio), y=log(height)), pch = 1, size = 2, alpha = 0.5)+
  geom_abline(intercept = coef(tree_height_gls)[1], slope = coef(tree_height_gls)[2],color = "black")+
  labs(y = "log (Tree Height)", x = " ")+
  annotate(geom= 'text', x=3.2, y=4, label=expression(paste(italic("R = "),"-0.11, ",italic("p = "),"3.5e-01")), parse = TRUE)+
  theme_classic()+
  theme(text = element_text(size=12),
        axis.title = element_text(face = "bold", size =12),
        strip.background =element_rect(fill="transparent", color="black", linewidth = 1),
