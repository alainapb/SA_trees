...
for (i in 1:nrow(trait_data)) {
  ...
  j%>%
   summarise(mean(height), na.rm = TRUE) %>% unlist() -> trait_data$treeheight[i]
  
  j%>%
   summarise(mean(length), na.rm = TRUE) %>% unlist() -> trait_data$leaflength[i]
  
  j%>%
   summarise(mean(width), na.rm = TRUE) %>% unlist() -> trait_data$leafwidth[i]
  
   j%>%
   summarise(mean(ratio), na.rm = TRUE) %>% unlist() -> trait_data$leafshape[i]
   
    j%>%
   summarise(mean(leaf_area_estimated), na.rm = TRUE) %>% unlist() -> trait_data$leafarea[i]
}
