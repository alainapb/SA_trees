...
#modis1<-raster("data/NPP_AET_data/MODIS/MOD16A2.006_ET_500m_doy2012001_aid0001.tif")

#m1<-extract(modis1, SA_grids) #extract data from raster
#str(m1) #check structure, 1207 grids with 14400 data points each... 

#saveRDS(m1, "output/AET_data.rds")
##read in ses.mpd results
AET<-readRDS("output/AET_data.rds") 
...
#saveRDS(output/gbm.fit2, "gbm.fit2.rds")
...
We don't need to use the chunk below because the second model was a great fit
```{r bgr_find_best_parameters, eval = FALSE}
...
hyper_grid <- expand.grid(
  ...
  bag.fraction = c(.65, .8),   # stochastic gradient descent
...
#  
#  saveRDS(hyper_grid, "output/hyper_grid.rds")
#}

hyper_grid<-readRDS("output/hyper_grid.rds")

#show top models 
hyper_grid %>% 
  dplyr::arrange(min_RMSE) %>%
  head(10)
#adjust parameters in hyper_grid to get better models, but these models are worse than gbm.fit2
