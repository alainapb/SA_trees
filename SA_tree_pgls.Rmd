...
for (i in 1:nrow(trait_data)) {
  ...
  j%>%
   summarise(median(height), na.rm = TRUE) %>% as.numeric -> trait_data$treeheight[i]
  
  j%>%
   summarise(median(length), na.rm = TRUE) %>% as.numeric -> trait_data$leaflength[i]
  
  j%>%
   summarise(median(width), na.rm = TRUE) %>% as.numeric -> trait_data$leafwidth[i]
  
   j%>%
   summarise(median(ratio), na.rm = TRUE) %>% as.numeric -> trait_data$leafshape[i]
   
    j%>%
   summarise(median(leaf_area), na.rm = TRUE) %>% as.numeric -> trait_data$leafarea[i]
}
...
right_join(richness_grid, trait_data, by = "ID")->trait_grid

trait_grid$count<-unlist(trait_grid$count)
trait_grid$treeheight<-unlist(trait_grid$treeheight)
trait_grid$leaflength<-unlist(trait_grid$leaflength)
trait_grid$leafwidth<-unlist(trait_grid$leafwidth)
trait_grid$leafshape<-unlist(trait_grid$leafshape)
trait_grid$leafarea<-unlist(trait_grid$leafarea)
