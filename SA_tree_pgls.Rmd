...
ggplot(mpd_grid) +
  ...
  scale_fill_scico(palette = "davos", direction = -1, end = 0.9, name = "quantiles (p) of SES") +
  geom_polygon(use, mapping=aes(lon, lat, group = group), fill = "transparent", colour = "black", size = 0.3)+
  theme_bw()+
  theme(
    #plot.background = element_rect(fill = "#f1f2f3"),
...
```{r NPP_from_1996, include=FALSE, echo =FALSE}
...
summary(lm(leafshape ~ NPP, data = test1))
cor.test(test1$leafshape, test1$NPP) #r = -0.578
plot(leafshape ~ NPP, data = test1) #rounder leaves in areas with higher NPP

#Is there a correlation between tree height and species richness? YES
summary(lm(log(treeheight*1000) ~ ntaxa, data =test1)) 
cor.test(log(test1$treeheight*1000), test1$ntaxa) #r = 0.712
plot(log(treeheight*1000) ~ ntaxa, data =test1) #taller trees in areas with more species 
```
...
full_model_obs<-lm(mpd.obs ~ log(treeheight*1000) + leafshape + ntaxa + NPP, data = test1)
...
full_model_ses<-lm(mpd.obs.z ~ log(treeheight*1000) + leafshape + ntaxa + NPP, data = test1)
...
###
model_lat<-lm(mpd.obs.z ~ log(treeheight*1000) + leafshape + ntaxa + NPP + lat + long, data = test1)
summary(model_lat)
plot(model_lat)
...
#clean data
bgr_data<-test1[,c(1,2,7,12,16,17,19)]
...
points(mpd.obs.z ~ leafshape + ntaxa + lat, bgr_data)
```
