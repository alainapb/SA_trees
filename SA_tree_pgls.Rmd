...
summary(lm(treeheight ~ ntaxa, data =test1)) 
cor.test(test1$treeheight, test1$ntaxa) #r = 0.726
plot(treeheight ~ ntaxa, data =test1) #taller trees in areas with more species 
```
...
full_model<-lm(mpd.obs ~ log(treeheight*1000) + leafshape + ntaxa + NPP, data = test1)
summary(full_model)
plot(full_model)
...
globalMoran #0.79  the residuals of the model are positively autocorrelated
#plot the correlation
moran.plot(full_model$residuals, listw = nb2listw(neighbours, style = "W"))


###
model_lat<-lm(mpd.obs ~ log(treeheight*1000) + leafshape + ntaxa + NPP +lat + long, data = test1)
summary(model_lat)
plot(model_lat)
...
#clean data
bgr_data<-test1[,c(1,2,7,8,16,17,19)]
...
## [1] 5.779 the model is off by ~6 units of mpd
...
# results
caret::RMSE(pred, bgr_data$mpd.obs) #0.499
```
...
ggplot(bgr_data)+
  geom_point(aes(x=lat, y = mpd.obs))+
  geom_line(aes(x=lat, y =prediction))
```
